...
const FullText = () => {
    ...
    const fetchArticles = async () => {
        try {
            ...
            setTotalArticlesCount(Array.isArray(result.data.studies) ? result.data.studies.length : 0);
            const includedArticles = result.data.studies.filter(article => article.titleScreeningStatus === 'included');
    ...
    const handleScreen = async (articleId, bodyScreeningStatus) => {
        await axios.put(`http://localhost:3000/api/articles/${articleId}/body-screen`, { bodyScreeningStatus });
        setArticles(articles && articles.map(article => article._id === articleId ? { ...article, bodyScreeningStatus, needsThirdPartyReview: bodyScreeningStatus === 'maybe' } : article));
    };
    ...
    const getInitialViewArticle = (articles) => {
        return articles.find(article => article.bodyScreeningStatus === 'unreviewed') || null;
    };
    ...
    return (
        <div className='Articles'>
            <div className='header'>Showing {articles.length} filtered out of {totalArticlesCount} Articles</div>
                        ...
                        <div className='articles-sidebar-content'>
                            {Array.isArray(articles) && articles.map(article => (
                                <div key={article._id} className='sidebar-item'>
                                    ...
                                    <div className={`sidebar-item-content ${viewArticle?._id === article._id ? 'active-sidebar-item-content' : `non-active-sidebar-item-content ${article.bodyScreeningStatus}-article-item`} `} onClick={() => handleView(article)}>
                                        <div className='article-title'>{article.title}</div>
                                        <div className='article-date'>Date: {new Date(article.publicationDate).toLocaleDateString()}</div>
                                        <div className={`article-status ${article.bodyScreeningStatus}-article-status`}>{article.bodyScreeningStatus !== 'unreviewed' ? article.bodyScreeningStatus : ''}</div>
                                    </div>
                                </div>
                            ))}
                        </div>
