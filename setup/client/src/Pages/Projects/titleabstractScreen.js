...
const StudyList = ({ projectId }) => {
  ...
  const handleScreen = async (studyId, screeningStatus) => {
    await axios.patch(`/api/studies/${studyId}/screen`, { screeningStatus });
    setStudies(studies.map(study => study._id === studyId ? { ...study, screeningStatus, needsThirdPartyReview: screeningStatus === 'maybe' } : study));
  };
